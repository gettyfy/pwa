...
const FormButton = (props: IButton) => {
    const { analyticName, handleAction, buttonColor, buttonName} = props
    ...
    return (
        <Box as="button" width="16.5rem" rounded="md" bg={buttonColor || 'blue.500'} size="lg" color="white" px={4} h={8} {...props} onClick={() => handleClick(analyticName)}>
        {buttonName}
        </Box>
    )
}
